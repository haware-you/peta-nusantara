...
### Inputs / Fields
- **Era scrubber, stretched:** the same instrument at a second height, never a second timeline. A "Stretch" toggle pill grows the track from 62px to the packed height of the lane view (capped at 34vh, scrolling internally past that). Inside, the 29 entities fan into the four lanes DATA already carries, on the same x-mapping as the ruler and under the same gold thumb; a 1px Tribute Gold cursor at 55% runs the full stretched height so both halves are demonstrably one control. Bars are HTML, not SVG — the ruler beneath is drawn with `preserveAspectRatio="none"`, which would distort type, and map text must stay selectable. Each bar is a 13px button: a 16% category wash, a 1px category hairline, a mono 0.55rem name that drops out (visibility, so it stays measurable) when the bar is narrower than its label. Selection is the standard gold hairline and wash. **Bar ends are feathered by confidence tier** with a CSS mask — 2px at high, 9px at medium, 18px at low — so a hard end is only ever drawn where a date is actually known: the VOC's 1799 gets an edge, Majapahit's 1527 does not. Rows come from greedy interval packing with a 0.6% cushion, which means two bars sit on separate rows precisely when they were running at the same time.
